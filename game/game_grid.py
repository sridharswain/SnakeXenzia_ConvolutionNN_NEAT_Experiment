...
class GameGrid:
    def __init__(self):
        self.gameVector = [[0 for i in range(measurement.GRID_LENGTH_X)] for j in range(measurement.GRID_LENGTH_Y)]
        self.not_occupied = {(i, j) for i in range (measurement.GRID_LENGTH_X) for j in range(measurement.GRID_LENGTH_Y)}

    def pin(self, x, y):
        gridX = x // measurement.SNAKE_NODE_WIDTH
        gridY = y // measurement.SNAKE_NODE_HEIGHT
        if (gridX < measurement.GRID_LENGTH_X and gridY < measurement.GRID_LENGTH_Y):
